...
class SimCLRDataGenerator:
    ...
    def color_distortion(image, s=1.0):
        ...
        def color_jitter(x):
            ...
            x = tf.image.random_contrast(x, lower=1-0.2*s, upper=1+0.5*s)
            x = tf.image.random_saturation(x, lower=1-0.2*s, upper=1+0.5*s)
    ...
    def show_augmented_images(generator, num_pairs=5):
